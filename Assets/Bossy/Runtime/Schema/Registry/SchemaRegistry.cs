...
namespace Bossy.Schema.Registry
{
    ...
    public class SchemaRegistry
    {
        ...
        private readonly Dictionary<CommandSchema, ValidationResult> _warnings = new();
        ...
        public IEnumerable<CommandSchema> GetInvalidSchemas(bool includeWarnings = false)
        {
            var list = _invalidSchemas.Keys.ToList();
            
            if (includeWarnings)
            {
                list.AddRange(_warnings.Keys);
            }
        ...
        public ValidationResult GetValidationResult(CommandSchema schema)
        {
            ...
            return _invalidSchemas.TryGetValue(schema, out var result) ? 
                result : _warnings.TryGetValue(schema, out result) ? result :
